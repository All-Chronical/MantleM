...
@export var animLibs: Array[AnimationLibrary] = []

var _resolved: PackedStringArray = PackedStringArray()

func resolve(lib_key: String, anim_lib: AnimationLibrary) -> void:
	...
	for i in range(MOVE_CODES.size()):
		...
		if anim_lib != null and anim_lib.has_animation(anim_name):
